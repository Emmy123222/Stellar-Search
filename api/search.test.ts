...
describe("api/search — Vercel x402 settlement (aligned with Express)", () => {
  ...
  it("validates and forwards locale, country, and language params", async () => {
    const fakeTx = Buffer.from(
      JSON.stringify({ transactionHash: "locale_tx" }),
    ).toString("base64");
    let capturedBody: any = null;
    global.fetch = vi.fn().mockImplementation(async (_url: any, opts: any) => {
      capturedBody = JSON.parse(opts.body);
    ...
    const { req, res } = mockReqRes({
      method: "GET",
      query: { q: "stellar", locale: "en-US", country: "us", language: "en" },
      headers: { "x-payment": fakeTx },
    });

    await handler(req, res);
    expect(capturedBody.hl).toBe("en");
    expect(capturedBody.gl).toBe("us");
    expect(res._json.locale).toBe("en-US");
    expect(res._json.country).toBe("us");
    expect(res._json.language).toBe("en");
  });
