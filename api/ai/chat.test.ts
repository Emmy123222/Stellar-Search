...
describe("AI Chat Serverless & Express Contract Tests (api/ai/chat.ts)", () => {
  ...
  it("rejects client-supplied system messages and keeps them out of the trusted prompt", async () => {
    const { req, res, getStatusCode, getResponseData } = createMockReqRes({
      method: "POST",
      body: {
        messages: [
  ...
  it("adds trusted server context from the context field after the base system prompt", async () => {
    ...
    const { req, res, getStatusCode } = createMockReqRes({
      method: "POST",
      body: {
        context: "Context — latest search: query=stellar",
    ...
    expect(mockCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        messages: expect.arrayContaining([
          expect.objectContaining({
            role: "system",
            content: expect.stringContaining("You are StellarSearch AI"),
          }),
          expect.objectContaining({
            role: "system",
            content: "Context — latest search: query=stellar",
          }),
          expect.objectContaining({
            role: "user",
