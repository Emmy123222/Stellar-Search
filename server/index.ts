...
export function validateSearchLocalization(
  input: Record<string, unknown> = {},
):
  | { ok: true; values: { locale: string; country: string; language: string } }
  | { ok: false; error: string } {
  const locale =
    (typeof input.locale === "string" ? input.locale : "en-US").trim() ||
    "en-US";
  const country =
    (typeof input.country === "string" ? input.country : "us")
      .trim()
      .toLowerCase() || "us";
  const language =
...
app.get("/search", async (req: Request, res: Response) => {
  ...
  const loc = validateSearchLocalization({ locale, country, language });
  if (!loc.ok) {
    const errorBody: ApiErrorResponse = { error: loc.error };
    return res.status(400).json(errorBody);
  }
  const {
...
app.get("/images", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errorBody: ApiErrorResponse = {
      error: "Image search failed. Check server logs.",
    };
    return res.status(500).json(errorBody);
  }
});
...
app.get("/news", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errorBody: ApiErrorResponse = {
      error: "News search failed. Check server logs.",
    };
    return res.status(500).json(errorBody);
  }
});
...
app.post("/search/batch", async (req: Request, res: Response) => {
  ...
  const parsedCount = Math.min(
    Math.max(parseInt(String(rawCount ?? "5")) || 5, 1),
    20,
  );
