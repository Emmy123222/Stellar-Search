...
The frontend records distinct latency phases for the paid search flow so the UI clearly distinguishes settlement work from provider work:

- **Challenge:** first `GET /search` response and 402 negotiation
- **Signing:** Freighter wallet auth-signing for the Soroban entry
- **Settlement:** x402 retry + confirmation path that settles the payment
- **Provider:** final provider search call and result fetch
- **Rendering:** results hydration and display in the browser

These timings are exposed in the `SearchSession` state and used by the payment flow visualizer, so the active and completed phases reflect real events instead of promise creation.
