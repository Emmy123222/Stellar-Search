...
Public AI chat clients may submit only `user` and `assistant` messages. Any client-supplied `system` messages are rejected at the API boundary. Server-generated search context is sent through a dedicated `context` field and inserted as a trusted system message after the base app prompt, so the app retains control over behavior while still allowing search-aware follow-ups.
